...
- **Recipe bodies are markdown under fixed section headings — rendered, not
  grammar-parsed.** ladle splits a body on its `## HEADING`s and hands each
  section to the in-repo, dependency-light block renderer (the one that already
  renders prose), instead of parsing steps/ingredients into a bespoke line
  grammar. One parse produces the HTML both outputs consume (WeasyPrint for PDF,
  pandoc `--from html` for EPUB), so they can't diverge. Markdown freedom is "we
  don't block it": a documented, tested subset (inline links/bold/italic, `-`/`*`
  and `1.` lists, `###` sub-headings) is styled; anything else renders at the
  author's risk. It follows that: headings are **fixed canonical English keys**
  (`## INGREDIENTS`, `## DIRECTIONS`, `## NOTES`) with localized display via the
  existing `labels:` map (so **no native-language heading alias map**); an
  **unrecognized heading renders as a generic titled block and warns** (nothing
  silently dropped, no per-theme "slot" contract); a **missing mandatory section
  warns, it does not block** the build (a drink or cheese board is legitimate;
  `validate --strict` can escalate); and **`##` is reserved as the section
  boundary** while `###` and deeper are free inside.
